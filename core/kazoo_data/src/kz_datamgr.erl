...
get_results(DbName, DesignDoc, Options) when ?VALID_DBNAME(DbName) ->
    ...
    case kzs_view:get_results(Plan, DbName, DesignDoc, Options) of
        {'error', 'not_found'} -> maybe_refresh_views(DbName, Plan, DesignDoc, Options);
        Other -> Other
    end;
get_results(DbName, DesignDoc, Options) ->
    case maybe_convert_dbname(DbName) of
...
-spec maybe_refresh_views(kz_term:ne_binary(), map(), kz_term:ne_binary(), view_options()) -> get_results_return().
maybe_refresh_views(DbName, Plan, DesignDoc, Options) ->
    case props:is_true('view_update_tried', Options, 'false') of
        'true' -> maybe_create_view(DbName, Plan, DesignDoc, Options);
        'false' ->
            _ = refresh_views(DbName),
            kzs_view:get_results(Plan, DbName, DesignDoc, [{'view_update_tried', 'true'} | Options])
    end.
