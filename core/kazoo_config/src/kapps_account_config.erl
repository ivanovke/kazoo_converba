...
-type api_account() :: api_ne_binary() | kz_json:object().
...
%% @end
%%-----------------------------------------------------------------------------
-spec get_ne_binaries(api_account(), ne_binary(), kz_json:path()) -> api_ne_binaries().
...
%% @end
%%-----------------------------------------------------------------------------
-spec get_pos_integer(api_account(), ne_binary(), kz_json:path()) -> api_pos_integer().
...
-spec get_global(api_account(), ne_binary(), kz_json:path(), Default) ->
...
%% @end
%%--------------------------------------------------------------------
-spec get_hierarchy(api_account(), ne_binary(), kz_json:path()) -> kz_json:api_json_term().
...
%% @end
%%--------------------------------------------------------------------
-spec get_hierarchy(api_account(), ne_binary(), kz_json:path(), Default) ->
...
-spec get_with_strategy(ne_binary(), api_account(), ne_binary(), kz_json:path(), Default) ->
                               Default | kz_json:json_term().
...
%% @end
%%--------------------------------------------------------------------
-spec get_global_from_doc(ne_binary(), kz_json:path(), Default, kz_json:object()) ->
                                 Default | kz_json:json_term().
get_global_from_doc(Category, Key, Default, JObj) ->
    case kz_json:get_value(Key, JObj) of
        'undefined' ->
...
-spec load_config_from_system(api_ne_binary(), ne_binary()) -> kazoo_data:get_results_return().
...
config_origins(Doc, Acc) ->
    case {kz_doc:account_db(Doc), kz_doc:account_id(Doc)} of
        ...
        {'undefined', DocAccountId} ->
            Db = kz_term:format_account_db(DocAccountId),
...
account_id(?NE_BINARY=Account) -> kz_term:format_account_id(Account);
...
account_id_from_jobj(_JObj, 'false') ->
    lager:debug("unable to find account id from ~p", [_JObj]),
    'no_account_id'.

-spec maybe_format_account_id(api_ne_binary()) -> account_or_not().
maybe_format_account_id('undefined') -> 'no_account_id';
maybe_format_account_id(Account) -> kz_term:format_account_id(Account).
    ...
    case kz_json:get_value(Key, JObj) of
        ...
        _Else ->
            io:format("the system tried to move the parameter listed below"
                      ...
                      "  Source~n    db: ~s~n    id: ~s~n    key: ~s ~s~n    value: ~p~n"
                      "  Destination~n    db: ~s~n    id: ~s~n    key: ~s ~s~n    value: ~p~n"
