...
add_account_name(#auth_user{account_id=AccountId}=AuthUser) ->
    case kzd_account:fetch(AccountId) of
        {'error', _} -> AuthUser;
        {'ok', Account} ->
            Realm = kzd_account:realm(Account),
            AuthUser#auth_user{account_name = kzd_account:name(Account)
                              ,account_realm = Realm
...
get_account_id(JObj) ->
    case get_account_db(JObj) of
        'undefined' -> 'undefined';
        AccountDb -> kz_term:format_account_id(AccountDb, 'raw')
    end.
...
get_account_db(JObj) ->
    ...
    of
        'undefined' -> 'undefined';
        AccountDb -> kz_term:format_account_id(AccountDb, 'encoded')
    end.
