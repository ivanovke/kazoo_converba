...
build_flow_data(Call, DataOut, AuthorizingType, Data)
  when AuthorizingType =:= <<"device">>;
    ...
    build_predtmf([{<<"approved_device_id">>, kapps_call:authorizing_id(Call)}
                   | DataOut
                  ], Data);
build_flow_data(Call, DataOut, <<"user">>, Data) ->
    build_predtmf([{<<"approved_user_id">>, kapps_call:authorizing_id(Call)}
                   | DataOut
                  ], Data);
build_flow_data(_Call, DataOut, _AuthorizingType, Data) ->
    lager:debug("unhandled authorizing type ~s", [_AuthorizingType]),
    build_predtmf(DataOut, Data).

-spec build_predtmf(kz_term:proplist(), kz_json:object()) -> kz_json:object().
build_predtmf(DataOut, Data) ->
    kz_json:from_list([{<<"queue_dtmf">>, kz_json:get_ne_binary_value(<<"queue_dtmf">>, Data)}
                       | DataOut
                      ]).
